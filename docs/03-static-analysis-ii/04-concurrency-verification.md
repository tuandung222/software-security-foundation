...
:::tip[DS perspective]
Data race trong systems code **chính là** phenomenon của **Hogwild! async SGD** trong distributed training - nhiều worker cùng update parameter mà không lock, race xảy ra. Paper Hogwild! (NIPS 2011) chứng minh "với learning rate nhỏ và sparsity, race tolerable". Đây là **same problem class** nhưng ML chấp nhận race với probabilistic bound, còn safety-critical software cần verify formal. Nếu bạn debug `torch.distributed` hoặc Ray training job, intuition về race condition đã có sẵn. **Memory model** (TSO, weak) tương tự **eventual consistency** trong distributed DB.
:::
